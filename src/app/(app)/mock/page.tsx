...
export default async function MockPage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("onboarding_completed, placement_completed, level")
    .eq("user_id", user.id)
    .single();
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-8 sm:px-8 md:py-10">
      ...
      {/* CTA — wired in Phase G */}
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-border bg-bg-card p-6 text-center shadow-sm">
        <p className="text-sm text-text-secondary">
          Full mock session UI is coming soon.
        </p>
        <Link
          href="/dashboard"
          className="inline-flex items-center rounded-2xl bg-primary px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-orange-500/20 transition-transform hover:scale-[1.02]"
        >
          Back to Home
        </Link>
      </div>
