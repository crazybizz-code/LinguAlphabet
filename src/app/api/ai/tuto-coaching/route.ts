...
import { generateTutoCoaching } from "@/ai/services/tuto-coaching";
...
export async function POST() {
  ...
  const rateLimit = checkRateLimit(`tuto-coaching:${user.id}`, RATE_LIMIT, RATE_WINDOW_MS);
  ...
  try {
    ...
    const result = await generateTutoCoaching(evidence);
    return NextResponse.json(result);
  } catch (error) {
    const message = error instanceof AIProviderError ? error.message : "Something went wrong generating coaching tips.";
