...
export function OnboardingWizard({ onComplete, destination }: OnboardingWizardProps) {
  ...
  return (
    // Warm off-white (--color-bg: #FFFDFB) instead of stark white (bg-bg-card)
    // — matches the design direction of a calm, premium consumer learning product.
    <div className="flex min-h-dvh flex-col bg-bg">
      ...
      {stepIndex > 0 && (
        <motion.button
          ...
          onClick={goBack}
          aria-label="Go back"
          className="fixed left-5 top-5 z-40 flex h-10 w-10 items-center justify-center rounded-full border border-border bg-bg/80 shadow-soft backdrop-blur-md transition-all hover:bg-bg hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring"
        >
          <ArrowLeft className="h-4 w-4 text-text-secondary" aria-hidden="true" />
        </motion.button>
      )}

      {/* Reduced vertical padding (py-14/sm:py-16 vs py-20) tightens the
          composition on desktop while keeping enough breathing room on mobile. */}
      <div className="flex flex-1 items-center justify-center overflow-y-auto px-5 py-14 sm:px-6 sm:py-16">
      ...
      {/* Step indicator: growing pill = completed/current, small dot = upcoming.
          Current step is bg-primary/60 (60% orange) so it reads as in-progress
          rather than being indistinguishable from completed (100% orange). */}
      <div
        className="flex justify-center gap-1.5 pb-8 max-md:pb-[calc(20px+env(safe-area-inset-bottom))]"
        aria-hidden="true"
      >
