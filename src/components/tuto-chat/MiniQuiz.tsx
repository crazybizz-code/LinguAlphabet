  ...
  return (
    <motion.div variants={fadeScaleIn} initial="hidden" animate="visible" className="rounded-2xl border border-border bg-bg-muted p-4">
          ...
          return (
            <button
              ...
              className={cn(
                ...
                revealed && isSelected && !isCorrectOption && "border-danger/40 bg-danger/5 text-text-primary",
            ...
            >
              ...
              {revealed && isSelected && !isCorrectOption && <X className="h-4 w-4 shrink-0 text-danger" aria-hidden="true" />}
            </button>
          );
        })}
      </div>
      {selected !== null && (
        ...
        >
          {selected === quiz.correctIndex ? "Nice work — that's it!" : "Not quite — the highlighted answer is correct."}
        </motion.p>
      )}
    </motion.div>
  );
