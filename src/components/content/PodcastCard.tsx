 ...
 * container either way, so layout never shifts. Mirrors ArticleCard's
 * identical hook exactly.
 */
...
export function PodcastCard({ podcast, tutosPick = false, index = 0 }: PodcastCardProps) {
  const thumbnail = useThumbnailState(podcast.thumbnailUrl);
      ...
      >
        <div className="relative aspect-square overflow-hidden">
          {thumbnail.tier === "fallback" ? (
            <BrandedCoverFallback contentType="podcast" />
          ) : (
            <Image
