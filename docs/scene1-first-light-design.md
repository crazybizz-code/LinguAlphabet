...
> 2. **Composition, not just scale, changes at desktop widths.** Top-anchoring the conversation exists for a real portrait/mobile constraint (keeping the orb clear of a software keyboard or a thumb reaching up a tall narrow screen) — it is not a constraint on a landscape desktop window, where it reads as a small mobile column adrift over a dead lower two-thirds of the browser. From `min-width: 880px`, the whole conversation re-composes around the vertical center of the viewport and the atmosphere (`.ds-vignette`, `.ds-veil`) re-anchors with it, so the warm wash frames Tuto at his new position instead of lingering where the old top-anchored composition used to sit. Verified at 375/834/1440/1920px: mobile and portrait-tablet keep the original top-anchored composition unchanged; laptop and desktop recompose around the vertical center with proportionally larger Tuto, type, and reply bubble.
>
