...
comment on column public.podcast_details.transcript_provenance is
  'publisher | publisher_episode_page | operator | generated_asr. Load-bearing for trust, not display: a publisher-supplied transcript deserves a different verification bar than an operator-typed one, and a machine-transcribed episode (generated_asr) must never be mistaken for one the publisher wrote. publisher_episode_page is the publisher''s own text taken from the episode page rather than the feed — same authority, more fragile extraction, so it is tracked separately.';
